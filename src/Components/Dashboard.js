...
import { Box, CircularProgress, IconButton, Container, Typography, Grid, Card, CardContent, TextField } from '@mui/material';
...
const Dashboard = () => {
  ...
  const handleTouchStart = (id, dateStr, name) => {
    // Set a timeout to trigger the delete alert after 0.5 seconds
    const timeout = setTimeout(() => {
      setSelectedItemId(id)
      setSelectedItemDate(dateStr)
  ...
  const handleTouchEnd = () => {
    // Clear the timeout if the touch is released before 0.5 seconds
    if (touchStart) {
                              ...
                              {(filters.type === "All" || filters.type === item.type) && (searchString === "" || item.itemName.toLowerCase().includes(searchString.toLowerCase())) && (
                                <>
                                  <Card onClick={() => {
                                    ...
                                    onTouchStart={() => handleTouchStart(item.itemId, date, item.itemName)}
